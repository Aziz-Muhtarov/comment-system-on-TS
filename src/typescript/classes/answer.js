class Answer {
    ...
    setUser(idx) {
        ...
        this.userNextAnswer.setAttribute("data-rating", `${this.rating.displayRatingAnswer(this.userNextAnswer.dataset.index)}`);
        this.userNextAnswer.innerHTML = `
          <img src="${this.main.answers[idx].src}" alt="user" width="61" height="61"/>
          ...
          <p class="comments__answer-text-reply">
            <img src="./src/assets/reply.svg" alt="reply"/>
              <span class="comments__answer-reply">${this.main.answers[idx].toWhom}</span>
          </p>
        ...
        this.targets.forEach((el) => {
            ...
            const countElement = this.userNextAnswer.querySelector(".comments__count");
    ...
    commentAnswer() {
        this.buttonAnswers = document.querySelectorAll(".comments__archive-answer-btn");
        this.buttonAnswers.forEach((el) => {
            el.addEventListener("click", () => {
                this.authorAnswerName = document.querySelector(".comments__answer-title-nav");
                this.authorAnswerImg = document.querySelector(".authorImg");
                this.buttonAnswer = el.parentElement;
                this.toWhomAnswerName = this.buttonAnswer.querySelector(".comments__answer-title")?.textContent || null;
            ...
            `;
                ...
                this.textarea = this.commentsFormsElements.namedItem("comment");
                this.output = this.userNavAnswer.querySelector(".comment__output-form");
                this.buttonSubmit = this.userNavAnswer.querySelector(".comment__input-btn");
                ...
                this.userNavAnswer.addEventListener("submit", (event) => {
                    if (this.textarea.value.length === 0 ||
                        ...
                        localStorage.setItem("answers", JSON.stringify(this.main.answers));
                        this.setNextAnswers();
                        this.buttonAnswerClose();
                    }
                });
