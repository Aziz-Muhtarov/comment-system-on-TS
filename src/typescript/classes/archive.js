class Archive {
    ...
    setNextUser(idx) {
        ...
        this.userNextComment.setAttribute("data-rating", `${this.rating.displayRatingArchive(this.userNextComment.dataset.index)}`);
        this.userNextComment.innerHTML = `
        <img src="${this.main.users[idx].src}" alt="user" width="61" height="61"/>
        <p class="comments__answer-title">${this.main.users[idx].first} ${this.main.users[idx].last}</p>
      ...
      `;
        ...
        const ratingSpan = this.userNextComment.querySelector(".comments__count");
